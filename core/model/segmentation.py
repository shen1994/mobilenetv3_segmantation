...
def get_mobilenetv3_large_seg(classnum=5, pretrained=False, root='save',
                              pretrained_base=False, **kwargs):

    model = MobileNetV3Seg(classnum, backbone='mobilenetv3_large',
...
def get_mobilenetv3_small_seg(classnum=5, pretrained=False, root='save',
                              pretrained_base=False, **kwargs):

    model = MobileNetV3Seg(classnum, backbone='mobilenetv3_small',
                           pretrained_base=pretrained_base, **kwargs)
    if pretrained:
        model.load_state_dict(torch.load('save/mobilenetv3_small.pth'))
    return model
