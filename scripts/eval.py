...
class Evaluator(object):
    ...
    def eval(self):
        ...
        for i, (image, target) in enumerate(self.val_loader):
            #oimage = cv2.imread("/home/westwell/Desktop/mobilenetv3-segmentation/dataset/rgb/train/1.png", 1) 
            #image = cv2.resize(oimage, (768, 768), interpolation = cv2.INTER_LINEAR)
            #cv2.imshow("image", np.array(image))
            #cv2.waitKey(1000)
            #oimage = np.transpose(oimage, (2, 0, 1)).astype(np.float32)
            #print(oimage.shape)
