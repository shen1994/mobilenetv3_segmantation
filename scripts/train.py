...
def parse_args():
    ...
    parser.add_argument('--classnum', type=int, default=6, 
                        help='class number of label dataset')
...
class Trainer(object):
    def __init__(self, args):
        ...
        trainset = get_segmentation_dataset(root=args.dataset, split='train', mode='train', **data_kwargs)
        ...
        if not args.skip_val:
            valset = get_segmentation_dataset(root=args.dataset, split='val', mode='val', **data_kwargs)
        ...
        self.model = get_segmentation_model(args.model, classnum=args.classnum,
...
def save_checkpoint(model, args, is_best=False):
    ...
    filename = '{}.pth'.format(args.model)
    ...
    if is_best:
        best_filename = '{}_best_model.pth'.format(args.model)
...
if __name__ == '__main__':
    ...
    logger = setup_logger("mobilenetv3_segmentation", args.log_dir, get_rank(), filename='{}_log.txt'.format(
        args.model))
